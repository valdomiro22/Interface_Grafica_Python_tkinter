...
def deletar():
    try:
        iten_selecionado = tv.selection()[0]
        tv.delete(iten_selecionado)
    except:
        messagebox.showinfo(title='ERRO', message='Selecione o item a ser deletado')


def obter():
    try:
        iten_selecionado = tv.selection()[0]
        valores = tv.item(iten_selecionado, 'values')
        print(f'ID: {valores[0]}    NOME: {valores[1]}     TELEFONE: {valores[2]}')
    except:
        messagebox.showinfo(title='ERRO', message='Selecione o item a ser mostrado')
